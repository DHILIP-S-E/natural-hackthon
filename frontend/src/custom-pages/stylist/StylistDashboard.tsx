...
// Helper: get step duration in seconds from a SOP step object
function getStepDurationSeconds(step: any): number {
  if (!step) return 300;
  const mins = step.duration_minutes ?? step.time_minutes ?? step.duration ?? 5;
  return Math.round(Number(mins) * 60) || 300;
}
...
export default function StylistDashboard() {
  ...
  // Bug 7: On mount (or when active booking loads), initialize timer from session state
  useEffect(() => {
    if (!activeBooking) return;

    // Restore current step from session
    const savedStep = activeBooking.current_step;
    if (savedStep && typeof savedStep === 'number' && savedStep >= 1) {
      setCurrentStep(savedStep);
    }

    // Calculate remaining time for the current step based on started_at
    const sessionStep = savedStep ?? 1;
    const stepDef = (activeBooking.sop_steps || activeBooking.currentSop || [])[sessionStep - 1];
    const stepDurationSec = getStepDurationSeconds(stepDef);

    if (activeBooking.started_at) {
      const startedAt = new Date(activeBooking.started_at).getTime();
      const elapsedSec = Math.floor((Date.now() - startedAt) / 1000);
      const remaining = Math.max(0, stepDurationSec - elapsedSec);
      setTimerValue(remaining);
    } else {
      setTimerValue(stepDurationSec);
    }
  }, [activeBooking?.id]);
                ...
                <AnimatePresence mode="wait">
                  <motion.div key={currentStep} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
                    ...
                    <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                      {currentStep > 1 && <button className="btn btn-ghost btn-sm" onClick={() => { setCurrentStep(s => s - 1); setTimerActive(false); setTimerValue(getStepDurationSeconds(sopSteps[currentStep - 2])); }}>Back</button>}
                      <button className="btn btn-primary btn-sm" onClick={() => { if (currentStep < sopSteps.length) { setCurrentStep(s => s + 1); setTimerActive(false); setTimerValue(getStepDurationSeconds(sopSteps[currentStep])); } }}>
